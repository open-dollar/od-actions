...
import {SafeERC20} from '@openzeppelin-sol7/token/ERC20/SafeERC20.sol';
import {SafeMath} from '@openzeppelin-sol7/math/SafeMath.sol';
...
library Utils {
  ...
  function transferTokens(address token, address payable destination, uint256 amount) internal {
    if (amount > 0) {
      if (token == ETH_ADDRESS) {
        (bool result,) = destination.call{value: amount, gas: 10_000}('');
